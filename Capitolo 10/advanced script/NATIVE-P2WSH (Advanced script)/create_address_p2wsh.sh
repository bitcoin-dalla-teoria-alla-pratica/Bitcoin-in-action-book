...
#         <Abdul the Lawyer's Pubkey> OP_CHECKSIG
#         ($THIRD_CASE)
#   ENDIF
#       <Mohammed's Pubkey> <Saeed's Pubkey> <Zaira's Pubkey>
#       3
#       CHECKMULTISIG
#       ($SECOND_CASE)
# ELSE
...
PB_ABDUL_LAWYER=$(cat compressed_public_key_1.txt)
PB_MOHAMMED=$(cat compressed_public_key_2.txt)
PB_SAEED=$(cat compressed_public_key_3.txt)
PB_ZAIRA=$(cat compressed_public_key_4.txt)

#pb1 == Abdul the Lawyer's
#pb2 == Mohammed
#pb3 == Saeed
#pb4 == Zaira
...
####FIRST CASE
SEC=7776000 #90 days in seconds
BIN=$(512to2.py $SEC)
FLAG=`echo 'ibase=2; 0000000001000000'$BIN | bc`
SEQUENCE_HEX=`printf $(echo 'obase=16; '$FLAG'' | bc) | tac -rs ..`
SEQUENCE_LENGTH=$(char2hex.sh $(printf $SEQUENCE_HEX | wc -c))

FIRST_CASE=$SEQUENCE_LENGTH$SEQUENCE_HEX$OP_CHECKSEQUENCEVERIFY$OP_DROP"21"$PB_ABDUL_LAWYER$OP_CHECKSIG

####SECOND CASE
SECOND_CASE="21"$PB_MOHAMMED"21"$PB_SAEED"21"$PB_ZAIRA"53"$OP_CHECKMULTISIG

####THIRD CASE
## 30 Days
SEC=2592000 #30 days in seconds
BIN=$(512to2.py $SEC)
FLAG=`echo 'ibase=2; 0000000001000000'$BIN | bc`
SEQUENCE_HEX=`printf $(echo 'obase=16; '$FLAG'' | bc) | tac -rs ..`
SEQUENCE_LENGTH=$(char2hex.sh $(printf $SEQUENCE_HEX | wc -c))

THIRD_CASE=$SEQUENCE_LENGTH$SEQUENCE_HEX$OP_CHECKSEQUENCEVERIFY$OP_DROP"21"$PB_ABDUL_LAWYER$OP_CHECKSIG

##SCRIPT
WITNESS_SCRIPT=$OP_IF$OP_IF"52"$OP_ELSE$THIRD_CASE$OP_ENDIF$SECOND_CASE$OP_ELSE$FIRST_CASE$OP_ENDIF
