...
SEQUENCE=`printf $(padding.sh 8 $( echo 'obase=16; '$FLAG'' | bc) | tac -rs ..)`
...
##SECOND SIGNATURE
SIGNATURE_2=`openssl pkeyutl -inkey private_key_2.pem -sign -in WITNESS_V0_DIGEST.txt -pkeyopt digest:sha256 | xxd -p -c 256`
...
# 0 <Saeed's Sig> <Abdul AVVOCATO's Sig> FALSE TRUE
WITNESS_FIELD=$(printf "00"$SIGNATURELENGTH_2$SIGNATURE_2$SIGNATURELENGTH$SIGNATURE"000101"$WITNESS_SCRIPT_LENGTH$WITNESS_SCRIPT)
