...
#TIME=`forwardseconds.py +180` To get non-mandatory-script-verify-flag (Locktime requirement not satisfied) (code 64) error
#TIME=201 to get non-mandatory-script-verify-flag (Negative locktime) (code 64)

TIME=`forwardseconds.py`
LOCKTIME_PART=$(printf $(echo 'obase=16; '$TIME' ' | bc) | tac -rs ..)

LENGTH_LOCKTIME_PART=$(bash char2hex.sh $(printf $LOCKTIME_PART | wc -c))
