...
printf "\n\n \e[105m ######### mine 36 blocks #########\e[0m\n\n"
bitcoin-cli generatetoaddress 36 $ADDR_MITT >> /dev/null

printf "\n\n \e[42m ######### Block height >= "$BLOCKHEIGHT". Current block height:"$(bitcoin-cli getblockchaininfo | jq -r '.blocks')". Transaction is valid from BLOCKHEIGHT "$BLOCKHEIGHT" #########\e[0m\n\n"
bitcoin-cli sendrawtransaction $TX_DATA_SIGNED
