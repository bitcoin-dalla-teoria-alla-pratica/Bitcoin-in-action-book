...
ADDR_MITT=`cat address_P2WPKH.txt`
...
OUTPUT=$(printf $TX_DATA | cut -c 111-162)
echo "OUTPUT: "$OUTPUT
...
LOCKTIME_PART=$(printf $TX_DATA | cut -c 163-171)
...
printf $WITNESS_V0_DIGEST | xxd -r -p | sha256sum -b | xxd -r -p | sha256sum -b | xxd -r -p > signature.txt
...
SIGNATURE=`openssl pkeyutl -inkey private_key_1.pem -sign -in signature.txt -pkeyopt digest:sha256 | xxd -p -c 256`
...
SEGWIT_FLAG="01"
#SCRIPT_SIGLENGTH="00"
INPUT_OUTPUT=$(printf $TX_DATA | cut -c 9-162) #number input, txid utxo, vout, #script sig length, nSequence,number output, value, scriptPubKey
FIRST_PART=$TX_VERSION$SEGWIT_MARKER$SEGWIT_FLAG$INPUT_OUTPUT
...
TX_DATA_SIGNED=$FIRST_PART$WITNESS_FIELD_COUNT$WITNESS_FIELD$LOCKTIME_PART
