...
if [ "$#" -eq 0 ]; then
    echo "Password necessaria. passare il parametro come da esempio: $0 <password>"
    exit 1
fi
...
SEGWIT_FLAG="01"
#SCRIPT_SIGLENGTH="00"
INPUT=$(printf $TX_DATA | cut -c 9-92) #number input, txid utxo, vout , sequence
FIRST_PART=$TX_VERSION$SEGWIT_MARKER$SEGWIT_FLAG$INPUT
...
WITNESS_FIELD=$(printf $HEXLENGTH$HEX"00"$SIGNATURELENGTH$SIGNATURE$WITNESS_SCRIPT_LENGTH$WITNESS_SCRIPT)
WITNESS_FIELD_COUNT="04"
