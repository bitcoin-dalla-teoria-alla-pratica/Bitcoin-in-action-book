...
# Witness script: OP_1 <Public Key hash> <Public Key hash> <Public Key hash> 3 OP_CHECKMULTISIGVERIFY OP_SHA256 SECRET OP_EQUAL
...
SECRET=$(printf $(printf $1 | xxd -ps) | xxd -r -p | sha256sum -b | xxd -r -p | sha256sum -b | awk '{print $1}')

#OP_1 <Public Key hash> <Public Key hash> <Public Key hash> 3 OP_CHECKMULTISIGVERIFY OP_SHA256 SECRET OP_EQUAL
WITNESS_SCRIPT="5121"$(cat compressed_public_key_1.txt)"21"$(cat compressed_public_key_2.txt)"21"$(cat compressed_public_key_3.txt)"53AFAA20"$SECRET"87"
