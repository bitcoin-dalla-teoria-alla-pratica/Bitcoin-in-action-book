...
#Get P2SH-P2WPKH
ADDR_MITT=`cat address_p2sh_p2wsh_wrap_p2pkh.txt`
...
CHECKSUM=$(bitcoin-cli getdescriptorinfo "sh(wsh(pkh($PK)))" | jq -r .checksum)
bitcoin-cli importdescriptors '[{ "desc": "sh(wsh(pkh('$PK')))#'"$CHECKSUM"'", "timestamp": "now", "internal": true }]'
