...
CHECKSUM=$(bitcoin-cli getdescriptorinfo "sh(pk($(cat compressed_private_key_WIF_1.txt)))" | jq -r .checksum)
bitcoin-cli importdescriptors '[{ "desc": "sh(pk('$(cat compressed_private_key_WIF_1.txt)'))#'"$CHECKSUM"'", "timestamp": "now", "internal": true }]'
